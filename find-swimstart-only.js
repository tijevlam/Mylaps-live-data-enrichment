...
// Finds chip codes (+ bib + name) of athletes who have a SwimStart passing
// but NEITHER a CheckIn passing NOR a SwimFinRunStart (swim exit / T1)
// passing -- i.e. SwimStart is the only one of the three they have. Useful
// for spotting check-in that got skipped/missed while the race is live.
//
//   node find-swimstart-only.js
//   node find-swimstart-only.js --raceType="middle distance"
//   node find-swimstart-only.js --checkin=CheckIn --swimstart=SwimStart --swimexit=SwimFinRunStart
//
...
async function main() {
    ...
    const [checkinChips, swimstartChips, swimexitChips] = await Promise.all([
        chipsAtSource(client, CHECKIN_SOURCE, null),
        chipsAtSource(client, SWIMSTART_SOURCE, RACE_TYPE),
        chipsAtSource(client, SWIMEXIT_SOURCE, null),
    ]);

    const swimstartOnly = [];
    for (const [chip, messageId] of swimstartChips) {
        if (checkinChips.has(chip)) continue;
        ...
        swimstartOnly.push({ chip, bib: bib || '', name: name || '' });
    }

    await client.quit();

    console.log(`raceType=${JSON.stringify(RACE_TYPE)}  checkin=${CHECKIN_SOURCE}  swimstart=${SWIMSTART_SOURCE}  swimexit=${SWIMEXIT_SOURCE}`);
    console.log(`${swimstartOnly.length} athlete(s) with ONLY a swim start passing (no check-in, no swim exit):\n`);
    for (const { chip, bib, name } of swimstartOnly) {
        console.log(`${chip}\t${bib}\t${name}`);
    }
}

main().catch(err => {
    console.error('Failed to look up swim-start-only athletes:', err);
    process.exit(1);
});
