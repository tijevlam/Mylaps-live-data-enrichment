...
// Socket.IO connection
    io.on('connection', (iosocket) => {
        ...
        db.all(`SELECT * FROM messages ${roomName && roomName != "everywhere" ? `WHERE sourceName LIKE "%${roomName}%"`: ""} ${query.bibnr ? `AND Bib = ${query.bibnr}` : ""} ${query.laps ? `AND l > ${query.laps}` : ""}  ORDER BY timestamp DESC LIMIT 30;`, (err, rows) => {
            if (err) {
